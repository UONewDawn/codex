![Lumberjacking](../../assets/Flag_lumberjacking.gif){ align=right }

# Lumberjacking

Harvest wood from trees and enhance axe damage.

## Overview

Lumberjacking allows you to chop trees for logs and provides a damage bonus when using axe weapons. Essential for crafters who need wood and beneficial for axe warriors.

## Effects

### Resource Gathering
- Chop trees for logs
- Different tree types yield different wood (if available)
- Logs convert to boards for crafting
- Renewable resource

### Combat Bonus
When using axe weapons:
- Damage bonus to axe attacks
- Stacks with other combat skills
- Makes axes more viable

## Training

### Early (0-30)
- Buy an axe from NPC
- Chop any trees
- Practice near towns

### Intermediate (30-60)
- Chop in forests
- Gather while training
- Build log stockpile

### Advanced (60-85)
- Chop special trees (if available)
- Master lumberjacking
- Efficient gathering

### Master (85-100)
- Expert woodsman
- Maximum resource yield
- Best combat bonus

## Materials

### Tools
- **Axe** (any type) - Required for chopping
- Hatchet, Battle Axe, Double Axe, etc.

### Gathering Locations
- Forests
- Tree-filled areas
- Any outdoor trees

## Related Skills

**Essential For:**
- [Bowcraft/Fletching](bowcraft-fletching.md) - Needs logs ⭐⭐⭐⭐⭐
- [Carpentry](carpentry.md) - Needs boards ⭐⭐⭐⭐⭐

**Combat Synergy:**
- [Swordsmanship](swordsmanship.md) - If using axes
- [Tactics](tactics.md) - Damage multiplier
- [Anatomy](anatomy.md) - More damage

**Lumberjack Crafter:**
- Lumberjacking: 100
- [Carpentry](carpentry.md): 100 or [Bowcraft/Fletching](bowcraft-fletching.md): 100
- [Magery](magery.md): 100
- Other utility skills

## Wood Types

### Normal Wood
- Most common
- Found everywhere
- Standard quality

### Special Wood (if available)
- Oak, Ash, Yew, Heartwood
- Rare spawn locations
- Enhanced properties
- Better crafted items

## Economic Value

### Income Sources
- Sell logs to crafters
- Sell boards
- Supply bowcrafters and carpenters
- Special wood premium pricing

### Demand
- High demand from crafters
- Consistent need
- Good supplemental income

## Combat Builds

### Lumberjack Warrior
- [Swordsmanship](swordsmanship.md): 100 (for axes)
- Lumberjacking: 100
- [Tactics](tactics.md): 100
- [Anatomy](anatomy.md): 100
- [Healing](healing.md): 100
- [Resisting Spells](resisting-spells.md): 100
- [Parrying](parrying.md): 100

## Importance

⭐⭐⭐⭐⭐ (Essential for wood crafters)
⭐⭐⭐ (Good for axe warriors)

---

**Related Pages:**
- [Carpentry](carpentry.md)
- [Bowcraft/Fletching](bowcraft-fletching.md)
- [Swordsmanship](swordsmanship.md)
- [Resources](../../items/resources.md)
