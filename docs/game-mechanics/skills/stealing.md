![Stealing](../../assets/Flag_stealing.gif){ align=right }

# Stealing

Take items from other players and NPCs.

## Overview

Stealing allows you to take items from other players' backpacks and NPCs. A high-risk, high-reward skill that enables thief gameplay. Server rules vary significantly for thievery.

## Effects

### Item Theft
- Steal from player backpacks
- Take from NPC inventory
- Success based on skill and item weight
- Immediate criminal flag

### Weight Limits
Success based on:
- Your stealing skill
- Item weight
- Target's awareness
- Random chance

## Training

### Early (0-30)
- Steal from NPCs in non-guard areas
- Practice on easy targets
- Light items first

### Intermediate (30-60)
- Steal heavier items
- Practice on various targets
- Improve success rate

### Advanced (60-85)
- Steal valuable items
- Better success rates
- Fewer failures

### Master (85-100)
- Expert thief
- Steal heavy items
- Maximum success rate

!!! warning "Criminal Activity"
    Stealing makes you criminal immediately. Guards will attack in town.

## Related Skills

**Essential Thief Skills:**
- [Snooping](snooping.md) - View before stealing ⭐⭐⭐⭐⭐
- [Hiding](hiding.md) - Escape after theft ⭐⭐⭐⭐⭐
- [Stealth](stealth.md) - Approach undetected ⭐⭐⭐⭐⭐

**Thief Template:**
- Stealing: 100 ⭐
- [Snooping](snooping.md): 100
- [Hiding](hiding.md): 100
- [Stealth](stealth.md): 100
- [Wrestling](wrestling.md): 100
- [Magery](magery.md): 80
- [Resisting Spells](resisting-spells.md): 100

## Uses

### Player Theft
- Steal valuables from players
- Target specific items
- High risk, high reward
- Requires excellent execution

### NPC Theft
- Steal from merchant NPCs
- Lower risk
- Practice and profit
- Moderate rewards

## Criminal System

### Instant Criminal Flag
- Stealing makes you criminal
- Guards attack on sight
- Other players can attack freely
- Criminal timer must expire

### Murder Counts
- Stealing doesn't give murder counts
- But being killed as a thief has no penalty reduction
- Still considered criminal activity

## Strategy

### Successful Theft
1. Scout target ([Snooping](snooping.md))
2. Approach hidden ([Hiding](hiding.md) + [Stealth](stealth.md))
3. Steal valuable item
4. Hide immediately
5. Stealth away
6. Leave area

### Target Selection
- Look for valuable items
- Assess risk level
- Plan escape route
- Know when to abort

### Escape Tactics
- Have recall ready
- Know hiding spots
- Use stealth effectively
- Multiple escape plans

## Server Rules

### Important Notes
- Stealing rules vary by server
- Some servers restrict or ban thief gameplay
- Check server rules before building a thief
- Penalties vary by shard

### Typical Rules
- Guard zones kill thieves instantly
- Criminal timer duration varies
- Some items may be unstealable
- Special thief guilds may exist

## Economic Value

### High Risk, High Reward
- Can steal extremely valuable items
- Significant profit potential
- Requires skill and planning
- Many failures and deaths

## Importance

⭐⭐⭐⭐⭐ (If thief gameplay is allowed)
⭐ (If severely restricted)

---

**Related Pages:**
- [Snooping](snooping.md)
- [Hiding](hiding.md)
- [Stealth](stealth.md)
- [PvP Guide](../pvp/index.md)
- [Murder System](../pvp/murder-system.md)
