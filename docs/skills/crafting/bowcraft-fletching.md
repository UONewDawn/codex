![Bowcraft/Fletching](../../../assets/Flag_bowcraft.gif){ align=right }

# Bowcraft/Fletching

## Overview

Bowcraft/Fletching allows you to craft ranged weapons and ammunition, essential for archers.

## Tools

In order to start crafting, you will need an Arrow Fletching tool, you can purchase one from bowyers vendors.

## Crafting list

These are all the ammunition and weapons you can craft.

=== "Materials"

    |                          Item                           |    Resources     | Skill |
    |:-------------------------------------------------------:|:----------------:|:-----:|
    | ![icon](../../../assets/items/kindling.png)<br>Kindling | 1 Boards or Logs |   0   |
    |    ![icon](../../../assets/items/shaft.png)<br>Shaft    | 1 Boards or Logs |   0   |

=== "Ammunition"

    |                           Item                            |          Resources           | Skill |
    |:---------------------------------------------------------:|:----------------------------:|:-----:|
    |     ![icon](../../../assets/items/arrow.png)<br>Arrow     | 1 Arrow Shafts<br>1 Feathers |   0   |
    | ![icon](../../../assets/items/bolt.png)<br>Crossbow Arrow | 1 Arrow Shafts<br>1 Feathers |   0   |
        
=== "Weapons"

    |                                Item                                 |     Resources     | Skill |
    |:-------------------------------------------------------------------:|:-----------------:|:-----:|
    |            ![icon](../../../assets/items/bow.png)<br>Bow            | 7 Boards or Logs  |  30   |
    |       ![icon](../../../assets/items/crossbow.png)<br>Crossbow       | 7 Boards or Logs  |  60   |
    | ![icon](../../../assets/items/heavy-crossbow.png)<br>Heavy Crossbow | 10 Boards or Logs |  80   |

## Repair deed

To craft a repair deed you will need a blank scroll, use your tool, click repair and target the scroll.

## Training

Consider Lumberjacking to fund the training.

| Skill    | Item            |
|----------|-----------------|
| 0 - 30   | Train from NPCs |
| 30 - 70  | Bows            |
| 70 - 90  | Crossbows       |
| 90 - 100 | Heavy Crossbow  |

## Related skills

- [Archery](../combat/archery.md)
- [Lumberjacking](../resource-gathering/lumberjacking.md)
